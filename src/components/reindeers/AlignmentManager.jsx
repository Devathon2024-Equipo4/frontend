...
export const AlignmentManager = () => {
  ...
  const { isLoading: isLoadingAlignments, Error: ErrorAlignments, alignments, fetchAlignments } = useGetAlignments();
  const { isLoading: isLoadingAlignmentRelation, Error: ErrorAlignmentRelation, fetchAlignmentRelation } = useGetAlignmentRelation();
  const [name, setName] = useState('');
  const {mutate: createAlignment, isPending: isCreateAlignment} = useCreateAlignment();
  const {mutate: createAlignmentRelation, isPending: isCreateAlignmentRelation} = useCreateAlignmentRelation();

  const handleSubmit = async (e) => {
    e.preventDefault();

    await createAlignment({name},{
      onSuccess: () => {
  ...
  const handleCreateAlignmentRelation = async (alignmentId) => {
    ...
    await createAlignmentRelation({data},{
      onSuccess: () => {
        toast.success(t('alignment.alignmentCreated'));
        fetchAlignments();
      },
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        <Input disabled={isCreateAlignment} label="Name" value={name} onChange={(e) => setName(e.target.value)}  placeholder={t('alignment.enterName')} />
      ...
      <ul>
        {alignments.map((alignment, ) => (
          <li key={alignment.id} className="flex items-center gap-x-2">
            <span className="cursor-pointer text-2xl" onClick={() => handleGetAlignmentsRelation(alignment.id)}>{alignment.name}</span>
            <Button onClick={() => handleEdit(alignment)} variant="outline">Edit</Button>
            <Button onClick={() => handleDelete(alignment.id)} variant="outline">Delete</Button>
            <Button onClick={ () => handleCreateAlignmentRelation(alignment.id)}>{t('alignment.saveAlignment')}</Button>
          </li>
        ))}
      </ul>
